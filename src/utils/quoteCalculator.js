...
export function calculateQuotes(pet) {
  ...
  if (pet.type === 'cat') {
    riskFactor *= 0.9
    factors.push(['Pet type', 'Cat rate', 0.9])
  } else {
    factors.push(['Pet type', 'Dog rate', 1])
  }
  ...
  const higherRiskBreeds = ['bulldog', 'german-shepherd']
  const breedFactor = higherRiskBreeds.includes(pet.breed) ? 1.2 : pet.breed === 'mixed' ? 0.95 : 1.05
  riskFactor *= breedFactor
  factors.push(['Breed profile', higherRiskBreeds.includes(pet.breed) ? 'Higher care profile' : pet.breed === 'mixed' ? 'Mixed breed' : 'Standard profile', breedFactor])
