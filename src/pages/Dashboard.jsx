            ...
            <div>
              ...
              <div className="mt-6 grid gap-6 xl:grid-cols-[1.3fr_.7fr]">
                <section className="rounded-3xl border border-[#25483a]/10 bg-white p-6 shadow-sm">
                  ...
                  {applications.length ? <div className="mt-5 divide-y divide-[#25483a]/10">{applications.slice(0, 3).map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-4 py-4"><div><p className="font-bold text-stone-800">{item.petName}</p><p className="mt-1 text-xs text-stone-500">{item.reference} · {formatDate(item.submittedAt)}</p></div><StatusPill tone="warm">{item.policyStatus}</StatusPill></div>)}</div> : <div className="mt-5"><EmptyState title="No applications yet" text="Complete your first pet insurance application to track it here." /></div>}
                </section>
            ...
            <div>
              ...
              {applications.length ? <div className="space-y-4">{applications.map((item) => (
                <article key={item.id} className="rounded-3xl border border-[#25483a]/10 bg-white p-6 shadow-sm">
                  ...
                  <div className="mt-6 grid gap-4 border-t border-[#25483a]/10 pt-5 text-sm sm:grid-cols-3"><div><p className="text-stone-500">Reference</p><p className="mt-1 font-bold">{item.reference}</p></div><div><p className="text-stone-500">Submitted</p><p className="mt-1 font-bold">{formatDate(item.submittedAt)}</p></div><div><p className="text-stone-500">Application status</p><p className="mt-1 font-bold">{item.status}</p></div></div>
