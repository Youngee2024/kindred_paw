...
export default function Dashboard() {
  ...
  const petBreedOptions = petForm.type ? [...(breedsByType[petForm.type] || [['', 'Select breed']])] : [['', 'Choose a pet type first']]
  if (petForm.breed && !petBreedOptions.some(([value]) => value === petForm.breed)) petBreedOptions.push([petForm.breed, petForm.breed])
  const petWeightOptions = [...weights]
  if (petForm.weight && !petWeightOptions.some(([value]) => value === petForm.weight)) petWeightOptions.push([petForm.weight, `${petForm.weight} kg`])
            ...
            <div>
              ...
              {applications.length ? <div className="space-y-4">{applications.map((item) => (
                <article key={item.id} className="rounded-3xl border border-[#25483a]/10 bg-white p-6 shadow-sm">
                  ...
                  <div className="mt-6 flex items-center gap-3"><span className={`size-3 rounded-full ${item.policyStatus === 'Active' ? 'bg-[#25483a]' : item.policyStatus === 'Declined' ? 'bg-[#9a4f2b]' : 'bg-[#c07a3f]'}`} /><div className="h-1 flex-1 rounded-full bg-[#f3dec8]"><div className={`h-full rounded-full ${item.policyStatus === 'Active' || item.policyStatus === 'Declined' ? 'w-full' : 'w-1/3'} ${item.policyStatus === 'Active' ? 'bg-[#25483a]' : 'bg-[#9a4f2b]'}`} /></div><span className="text-xs font-semibold text-stone-500">{item.policyStatus === 'Active' ? 'Policy active' : item.policyStatus === 'Declined' ? 'Application declined' : 'Review in progress'}</span></div>
                </article>
